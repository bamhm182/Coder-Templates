resource "libvirt_cloudinit_disk" "init_node0" {
  ...
  name       = lower("coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}-init-node0.iso")
  user_data  = data.template_file.user_data_node0[0].rendered
  pool       = "working"
}
...
resource "libvirt_volume" "root_node0" {
  name             = lower("coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}-k3s-node0.qcow2")
  count            = data.coder_workspace.me.start_count
  pool             = "working"
  format           = "qcow2"
...
resource "libvirt_volume" "home_node0" {
  count            = 1
  name             = lower("coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}-k3s-node0.home.qcow2")
  pool             = "working"
  format           = "qcow2"
...
resource "libvirt_domain" "node0" {
  name       = lower("coder-${data.coder_workspace_owner.me.name}-${data.coder_workspace.me.name}-k3s-node0")
  count      = data.coder_workspace.me.start_count
  memory     = (data.coder_parameter.ram_amount.value * 1024)
  vcpu       = data.coder_parameter.cpu_count.value
  qemu_agent = true
  cloudinit  = libvirt_cloudinit_disk.init_node0[0].id
  ...
  disk {
    volume_id = libvirt_volume.root_node0[0].id
  }

  disk {
    volume_id = libvirt_volume.home_node0[0].id
  }
...
resource "null_resource" "scripts_node0" {
  count = data.coder_workspace.me.start_count
  depends_on = [libvirt_domain.node0[0]]
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo ${coder_agent.kubernetes.token} > ~/.config/coder/token",
    ...
    connection {
      type        = "ssh"
      user        = "user"
      host        = flatten([for nic in libvirt_domain.node0[0].network_interface :
