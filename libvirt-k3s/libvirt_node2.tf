...
data "template_file" "user_data_node2" {
  ...
  template          = templatefile("${path.module}/cloudinit.cfg.template", {
    password_hash   = htpasswd_password.user_node2.sha512,
    authorized_keys = chomp(tls_private_key.ssh_key_node2.public_key_openssh),
  })
}
...
resource "libvirt_domain" "node2" {
  ...
  qemu_agent = true
  cloudinit  = libvirt_cloudinit_disk.init_node2.id
  ...
  disk {
    volume_id = libvirt_volume.root_node2.id
  }

  disk {
    volume_id = libvirt_volume.home_node2.id
  }
...
resource "null_resource" "scripts_node2" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type        = "ssh"
      user        = "user"
      host        = flatten([for nic in libvirt_domain.node2.network_interface :
                              [for addr in nic.addresses : addr if can(regex("^\\d+\\.\\d+\\.\\d+\\.\\d+$", addr))]])[0]
      private_key = tls_private_key.ssh_key_node2.private_key_openssh
      timeout     = "1m"
    }
  }
}
